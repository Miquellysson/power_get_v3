...
## 2025-11-06 — Tema alimentação, PWA e navegação
- Super administradores podem alternar o tema da vitrine (`settings.store_theme`) e gerenciar o tema alimentação pelo painel (`settings.theme_food_config`), incluindo textos, CTAs, cards e assets enviados para `storage/themes/food`.
- Banner PWA ganhou controle completo de exibição (`settings.pwa_banner_config`) e o painel permite habilitar/desabilitar seções da home (`settings.home_sections_visibility`).
- Administração de menus e páginas institucionais armazenada em `settings.navigation_header_links`, `settings.navigation_footer_links` e `settings.custom_pages`.
- Categorias passaram a aceitar ícones personalizados (Remix Icon ou Font Awesome) através da nova coluna `categories.icon` (VARCHAR(80), opcional). Execute:
  ```sql
  ALTER TABLE categories ADD COLUMN icon VARCHAR(80) NULL AFTER slug;
  ```
