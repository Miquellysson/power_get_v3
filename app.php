...
$appTitle = setting_get('pwa_name', 'App Get Power Research');
?>
<!doctype html>
<html lang="pt-BR">
<head>
  ...
  <link rel="apple-touch-icon" href="<?= htmlspecialchars($icon192); ?>">
...
<body>
  <div class="wrap">
    <div class="card">
      <img class="logo" src="<?= htmlspecialchars($icon192); ?>" alt="<?= htmlspecialchars($appTitle); ?>">
      <h1><?= htmlspecialchars($appTitle); ?></h1>
      <p class="lead">Adicione o <?= htmlspecialchars($appTitle); ?> à tela inicial para acesso rápido.</p>
      <button id="installBtn" class="primary" style="display:none">📲 Instalar App</button>
      <div class="hint" id="androidHint" style="display:none">Se o botão não aparecer, abra o menu ⋮ do Chrome e toque em <b>Instalar app</b>.</div>
      <button id="iosOpen" class="primary" style="display:none">📱 Como instalar no iPhone</button>
      <div class="hint" id="iosSmall" style="display:none">Disponível para Safari no iPhone/iPad.</div>
      <div class="divider"></div>
      <div class="hint">Este atalho funciona no domínio configurado da loja.</div>
    </div>
  </div>
  ...
  <div class="modal" id="mdl" aria-hidden="true">
    <div class="sheet" role="dialog" aria-modal="true" aria-labelledby="ttl">
      <div class="hero">
        <img src="<?= htmlspecialchars($icon192); ?>" alt="App">
        <h3 id="ttl">Instalar <?= htmlspecialchars($appTitle); ?> (iPhone)</h3>
      </div>
      <div class="body">
        <div class="steps">
          <div class="step">
            <span class="badge">1</span>
            <div class="text">Abra no <b>Safari</b> (este passo só funciona no Safari do iOS).</div>
          </div>
          <div class="step">
          ...
          <div class="step">
            ...
            <div class="text">Role e toque em <b>Adicionar à Tela de Início</b>.</div>
          </div>
          <div class="step">
            <span class="badge">4</span>
            <div class="text">Confirme o nome <b><?= htmlspecialchars($appTitle); ?></b> e toque em <b>Adicionar</b>.</div>
          </div>
        </div>
        <div class="actions">
          <button class="btn close" id="closeIos">Entendi</button>
        </div>
        <div class="legal">Dica: se o botão “Compartilhar” não aparecer, toque na barra para mostrá-lo.</div>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    if (isIOS && isSafari) {
      ...
      ovl.addEventListener('click', () => closeIos.click());
    }
