...
function admin_header($title='Admin'){
  ...
  $fullLogoPath = find_logo_path();
  $logoUrl = $fullLogoPath ? '/' . ltrim($fullLogoPath, '/') : null;
  $defaultPalette = [
      'DEFAULT' => '#2060C8',
      '50' => '#EEF4FF',
      '100' => '#DCE7FF',
  ...
  $logoHtml = $logoUrl
    ? '<div class="logo img"><img src="'.sanitize_html($logoUrl).'" alt="Logo" class="logo-img"></div>'
    : '<div class="logo icon"><i class="fa-solid fa-capsules"></i></div>';
  echo '<div class="brand">'.$logoHtml.'<div><div class="name">'.sanitize_html($store).'</div><div class="sub">Painel Administrativo</div></div></div>';
  ...
  if (function_exists('is_super_admin') && is_super_admin()) {
    echo '<a class="'.($cur==='backup.php'?'active':'').'" href="backup.php"><i class="fa-solid fa-cloud-arrow-down"></i>Backups</a>';
  }
