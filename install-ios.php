...
$iconRelative = function_exists('asset_url') ? asset_url('assets/icons/farma-192.png') : 'assets/icons/farma-192.png';
$appleIcon = (strpos($iconRelative, '/') === 0) ? $iconRelative : '/' . $iconRelative;
  ...
  <script>
    (function(){
      ...
      if ("serviceWorker" in navigator) {
        ...
        const SW_URL = <?=json_encode($swRegisterUrl)?>;
        window.__SW_URL__ = SW_URL;
        window.__APP_NAME__ = window.__APP_NAME__ || <?=json_encode($storeName)?>;
        window.__A2HS_ICON__ = window.__A2HS_ICON__ || <?=json_encode($appleIcon)?>;
        try { navigator.serviceWorker.register(SW_URL); } catch(e){}
        });
      }
    })();
  </script>
